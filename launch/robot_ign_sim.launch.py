...
def generate_launch_description():
    ...
    world_file = os.path.join(gazebo_pkg, 'worlds', 'default_ign.world')
    ...
    declare_sensors_flag = DeclareLaunchArgument(
        "SENSORS", 
        default_value="false", 
        description="Flag used to load sensors"
    )   

    declare_ign_flag = DeclareLaunchArgument(
        "ign", 
        default_value="true", 
        description="Flag used to load ign control plugin"
    ) 
    ...
    gazebo_ign_ld = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory("champ_simulation"),
                "launch",
